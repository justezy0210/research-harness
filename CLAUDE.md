...
- `eda-reports/`: 데이터 intake review 결과
- 필요한 경우 review note도 여기에 둘 수 있다
...
그다음 필요하면 `data-intake-review`를 먼저 수행한다.

`data-intake-review`의 목적:
- 파일 구조 확인
- 품질 확인
- 누락값/형식/기본 통계 확인
- 다음 단계가 build인지 analysis인지 판단
- 결과를 문서화

결과는 기본적으로 `00_docs/eda-reports/`에 저장한다.
...
필요하면 build 계획은 `research-consistency-check` 또는 `codex-orchestrator`로 검토한다.
...
필요하면 analysis 계획은 `research-consistency-check` 또는 `codex-orchestrator`로 검토한다.
...
methods 또는 results의 기술적 타당성을 점검하고 싶으면 `codex-orchestrator`를 사용할 수 있다.  
문헌 검색, novelty, competing work, reviewer-style 비평은 `manuscript-review`가 우선이다.
...
- research consistency check 결과
- Codex 검토 결과
- manuscript review 결과
- revision plan
- 제출 전 확인 메모
...
- build 또는 analysis 시작 전에 입력 상태를 문서화해야 할 때

결과는 기본적으로 `00_docs/eda-reports/`에 저장한다.

### `research-consistency-check`
다음 상황에서 사용한다.
- build 계획이 연구 질문과 맞는지 점검할 때
- analysis 계획과 실제 workflow의 정합성을 점검할 때
- 로그, 결과, manuscript 초안 사이의 연결이 맞는지 확인할 때
- 프로젝트 구조가 흐트러지기 시작했을 때

결과는 `06_review/` 또는 `00_docs/` 아래 review note로 남길 수 있다.
...
- analysis plan 검토
- 스크립트/코드 검토
- 폴더 구조 및 outputs 정리 상태 점검
- manuscript methods/results의 기술적 보조 검토
- 외부 모델 관점의 비판이 필요할 때

Codex 결과는 외부 검토 의견으로 취급한다.  
최종 판단은 Claude가 프로젝트 문서, 실행 기록, outputs, manuscript 상태를 종합해 내린다.

### `manuscript-review`
다음 상황에서 사용한다.
- manuscript 초안이 어느 정도 완성되었을 때
- reviewer 관점의 피드백이 필요할 때
- 최신 문헌, competing work, novelty framing을 점검해야 할 때
- 본문/표/그림/abstract 간 수치 일관성을 점검해야 할 때
...
따라서 무조건 많은 파일을 넣기보다, 현재 질문에 꼭 필요한 문맥만 선별해서 제공한다.
